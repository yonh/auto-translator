var background = function() {
  ...
  class TranslationCache {
    ...
    async init(maxAge) {
      ...
      const indexData = await browser$3.storage.local.get(CACHE_INDEX);
    ...
    async get(text, sourceLang, targetLang, model) {
      ...
      const data = await browser$3.storage.local.get(cacheKey);
    ...
    async set(text, sourceLang, targetLang, model, translatedText) {
      ...
      await browser$3.storage.local.set({ [cacheKey]: entry });
    ...
    async delete(hash) {
      const cacheKey = `${CACHE_PREFIX}${hash}`;
      await browser$3.storage.local.remove(cacheKey);
    ...
    async clear() {
      ...
      await browser$3.storage.local.remove([...cacheKeys, CACHE_INDEX]);
      this.cacheIndex.clear();
    }
    async saveIndex() {
      const indexObj = Object.fromEntries(this.cacheIndex);
      await browser$3.storage.local.set({ [CACHE_INDEX]: indexObj });
    }
    async cleanExpired() {
      const now = Date.now();
      const expired = [];
  ...
  background;
  function updateOpenAIConfig(config) {
  }
  background;
  ...
  class BackgroundTranslationManager {
    ...
    async loadSettings() {
      const data = await browser$2.storage.local.get("settings");
      this.settings = { ...DEFAULT_SETTINGS, ...data.settings };
    }
    async saveSettings() {
      await browser$2.storage.local.set({ settings: this.settings });
    }
    ...
    isUrlAllowed(url) {
      if (this.settings.whitelist.length > 0) {
        return this.settings.whitelist.some(
          (pattern) => url.includes(pattern)
        );
      }
      if (this.settings.blacklist.length > 0) {
        return !this.settings.blacklist.some(
          (pattern) => url.includes(pattern)
        );
      }
      return true;
  ...
  const translationManager = new BackgroundTranslationManager();
  background;
