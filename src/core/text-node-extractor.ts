...
export class TextNodeExtractor {
  ...
  private readonly excludeSelectors = [
    ...
    // Note: 'select' removed - we now translate select options
    "input",
    "textarea",
  ...
  extract(root: HTMLElement = document.body): TranslatableUnit[] {
    ...
    return units.filter(
      (unit) => unit.originalText.length >= this.config.minTextLength,
    );
  }

  /**
   * Extracts all translatable units including hidden content (like dropdowns).
   * This is useful for translating content that is initially hidden but will be shown later.
   */
  extractIncludingHidden(
    root: HTMLElement = document.body,
  ): TranslatableUnit[] {
    const textNodes = this.collectTextNodesIncludingHidden(root);
    const units = this.groupTextNodes(textNodes);
    return units.filter(
      (unit) => unit.originalText.length >= this.config.minTextLength,
    );
  ...
  /**
   * Collects all text nodes including hidden ones (for dropdowns, etc.).
   */
  private collectTextNodesIncludingHidden(root: HTMLElement): Text[] {
    ...
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
      acceptNode: (node: Text) => {
        ...
        // Skip visibility check for hidden content
        return NodeFilter.FILTER_ACCEPT;
      },
    });
   ...
   */
  private splitByInlineBoundaries(
    nodes: Text[],
    parent: HTMLElement,
  ): Text[][] {
    if (nodes.length <= 1) {
  ...
  private hasBoundaryBetween(
    ...
    if (
      this.blockElements.has(parent1.tagName.toLowerCase()) ||
